...
import { getTransactionReversalRequest, postTransactionReversalRequest, getClassDataRequest, getLocationDataRequest } from "../../Redux/Action/transactionReversal";
...
useEffect(()=> {
  ...
  useEffect(() => {
        if(editTransactionData?.data?.Data && Array.isArray(editTransactionData?.data?.Data)){
          ...
          setSubmit(false);
          setSearch(false);
        }else if(editTransactionData?.data?.itemData && Array.isArray(editTransactionData?.data?.itemData)){
          setItemData(editTransactionData?.data?.itemData);
          setOrigItemData(editTransactionData?.data?.itemData);
          setLoading(false);
        }else if(editTransactionData?.data?.locationData && Array.isArray(editTransactionData?.data?.locationData)){
    ...
    <Box className={editTransactionClasses.maindiv}>
      <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
        ...
        <Box display="flex"
              ...
              alignItems="flex-end" className={editTransactionClasses.boxDiv}>
            <div className={editTransactionClasses.uploaddiv}>
...
{/* 
      <Stack spacing={2} sx={{ width: "100%" }}>
        ...
        anchorOrigin={{
          ...
          <Alert
            ...
            severity={editTransactionClasses?.isSuccess ? "success" : "error"}
            sx={{ width: "100%" }}
          >
          {editTransactionClasses?.messgae}
          </Alert>
          </Snackbar>
      </Stack> */}
      {/* <Modal
      ...
      >
        <Box className={editTransactionClasses.popUp}>
